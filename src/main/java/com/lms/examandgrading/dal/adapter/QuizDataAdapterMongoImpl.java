...
import com.lms.examandgrading.dal.repository.QuizMongoRepository;
    ...
    public Quiz update(Quiz quiz) {
       QuizModel quizModel = mongoTemplate.findAndModify(Query.query(Criteria.where("qid").is(quiz.getQid())),
               new Update().set("question",quiz.getQuestion()).set("opt1",quiz.getOpt1()).set("opt2",quiz.getOpt2())
                       .set("opt3",quiz.getOpt3()).set("opt4",quiz.getOpt4()).set("answer",quiz.getAnswer()).set("teacherID",quiz.getTeacherID()).set("examID",quiz.getExamID()),
               QuizModel.class);
       return quiz;
    }
